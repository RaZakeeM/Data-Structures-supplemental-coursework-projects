...
static void treeFactory(GenericTree<int>& tree) {
    ...
    GenericTree<int>::TreeNode* four = tree.getRootPtr();
    four -> data = 4;
    GenericTree<int>::TreeNode* eight = four -> addChild(8);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  //use an intermediary data structure of pointers to solve the problem efficiently
  std::queue<TreeNode*> nodesToExplore;

 
    //first in is the root node
    nodesToExplore.push(rootNodePtr);
    //now we enter a while loop
    while (!nodesToExplore.empty()) {
      //grab the top pointer and copy it but clear out intermediary DS
      TreeNode* topNode = nodesToExplore.front();
      nodesToExplore.pop();
      //enter a for loop updating the intermediary for first line
      for (auto childPtr : topNode->childrenPtrs) {
        nodesToExplore.push(childPtr);
      }
        //release data into the returned structure from cleared top of intermediary
