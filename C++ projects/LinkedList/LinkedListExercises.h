...
void LinkedList<T>::insertOrdered(const T& newData) {
    ...
    Node * created_data_node = new Node(newData);
    
    
    if (!empty()){
    
    if(back() > front()){
    if( newData < back() && newData > front()){
    Node * der = tail_;
    int indexer = size();
    for (int i = 0; i< indexer -1; i++){
        int nodes_data = der -> data;
        der = der -> prev;
        nodes_data = der -> data;
        Node * red = der -> next;
        int node_next_data = red -> data;
        if (newData > nodes_data && newData < node_next_data){
            Node & upper_node = *red ;
            Node & lower_node = *der ;
           upper_node.prev = created_data_node;
            lower_node.next = created_data_node;
            created_data_node -> prev = &lower_node;
            created_data_node -> next = &upper_node;
            size_ = size_ +1;
        }
    }
    }
    if( newData > back()){
        created_data_node -> prev = tail_;
        tail_ -> next = created_data_node;
        tail_ = created_data_node;
        size_ = size_+1;
    }
    ...
    if( newData < front()){
        ...
        created_data_node -> next = head_;
        head_ -> prev = created_data_node;
        head_ = created_data_node;
        size_ = size_+1;
    }
    }
    ...
    if (empty()){
        created_data_node -> next = nullptr;
        created_data_node -> prev = nullptr;
        head_ = created_data_node;
        tail_ = created_data_node;
        size_ = size_+1;
    }
...
for (int i = 0 ; i < (other.size_ + (this -> size_)) ; i++){
if ((left.empty()) or (right.empty())){
            
        if ((left.empty()) and (!(right.empty()))){
            merged.pushFront(right.back());
            right.popBack();
    }
    
        if ((!(left.empty())) and (right.empty())){
            merged.pushFront(left.back());
            left.popBack();
    }
